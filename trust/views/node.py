...
def addNode(request):
    ...
    node.node_type = request.POST['node-type']
    node.node_file = request.POST['node-ip']
    node.node_active = request.POST['node-active']
    node.update_time = 0
    node.save()
    #getServer(node.name)
...
def add():
    ...
    ip = 123
    for k, v in geoCoordMap.items():
        node = Node()
        node.name = k
        node.introduction = 'none'
        node.node_ip = '192.0.168.' + str(ip)
        ip += 1
        node.location_x = float(v[0])
        node.location_y = float(v[1])
        node.node_type = '本地拉取'
        node.node_file = node.node_ip
        node.node_active = random.randint(0, 50) 
        node.update_time = int(time.time())
        node.save()
